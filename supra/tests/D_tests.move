...
module D::D_tests {
    ...
    fun test_warning_text_on_chain(deployer: &signer) {
        ...
        let supra_ref = b"Supra";
        assert!(contains_bytes(&w, &supra_ref), 201);
    ...
    #[test(deployer = @D)]
    #[expected_failure(abort_code = 15, location = D::D)]
    ...
    #[test(deployer = @D)]
    #[expected_failure(abort_code = 16, location = D::D)]
    ...
    /// MIN_DEBT for D Supra is 0.01 D (1_000_000 raw, 8 dec) — lowered from D Aptos'
    /// 0.1 D to match SUPRA's smaller-unit economics.
    #[test(deployer = @D, user = @0xA11CE)]
    #[expected_failure(abort_code = 6, location = D::D)]
    fun test_redeem_below_min_debt_aborts(deployer: &signer, user: &signer) {
        setup(deployer);
        // 0.009 D = 900_000 < MIN_DEBT (1_000_000)
        D::redeem(user, 900_000, @0xBEEF);
    }
}

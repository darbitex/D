...
**Goal:** Publish D package to Sui mainnet + seal permanently in two transactions.
...
- `currency_id` — `Currency<D>` sitting as TTO at `0xc` (CoinRegistry singleton)
- `currency_version` + `currency_digest` — needed to construct `Receiving<Currency<D>>` in Tx 2
...
1. `0x2::coin_registry::finalize_registration<PACKAGE_ID::D::D>(&mut 0xc, Receiving<Currency<D>>(currency_id, version, digest), ctx)` — promotes Currency<D> to shared
2. `PACKAGE_ID::D::destroy_cap(OriginCap, &mut Registry, UpgradeCap, &Clock@0x6, ctx)` — consumes both caps, `make_immutable`, flips `sealed = true`
...
- `Currency<D>` promoted to shared object (derived address)

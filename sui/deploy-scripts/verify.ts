 ...
 *   - Currency<D> promoted to shared object
...
async function main() {
    ...
    console.log('=== Verifying D Sui sealed state ===\n');
    ...
    console.log(
        `4. Currency<D> shared     = ${isShared}  ${isShared ? '✓' : '✗'}`
    );
    if (!isShared) {
    ...
    console.log(allOk ? '✓ D Sui is LIVE + SEALED.' : '✗ One or more checks failed — inspect above.');

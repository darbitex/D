 ...
 *   1. 0x2::coin_registry::finalize_registration<PKG::D::D>(
 *        &mut CoinRegistry@0xc,
 *        Receiving<Currency<D>>(currency_id, version, digest),
 *      )
 *   2. PKG::D::destroy_cap(OriginCap, &mut Registry, UpgradeCap, &Clock@0x6)
 *
...
async function main() {
    ...
    // Call 1: finalize_registration<PKG::D::D>(&mut CoinRegistry, Receiving<Currency<D>>)
    // Type argument is the D coin type
    const dType = `${out.package_id}::D::D`;
    ...
    tx.moveCall({
        target: `${out.package_id}::D::destroy_cap`,
        arguments: [
    ...
    console.log('\n=== Ready to execute ===');
    console.log(`This will PERMANENTLY SEAL the D package at ${out.package_id}.`);
    console.log(`Consumes OriginCap ${out.origin_cap_id}`);
    console.log(`Consumes UpgradeCap ${out.upgrade_cap_id}`);
    console.log(`Promotes Currency<D> ${out.currency_id} to shared object.`);
    ...
    console.log('\n✓ D Sui v0.1.0 is LIVE and SEALED on mainnet.');

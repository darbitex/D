...
# D Sui — Tx 1 publish
#
# Publishes the D package to Sui mainnet. Extracts created object IDs
# (Registry, OriginCap, UpgradeCap, Currency<D>) from the tx effects
...
if [ "$ACTIVE_ENV" != "mainnet" ] && [ "$ACTIVE_ENV" != "0" ] && [ "$ACTIVE_ENV" != "testnet" ]; then
    echo "ERROR: sui client active-env is '$ACTIVE_ENV', expected 'mainnet' or 'testnet'"
    echo "Run: sui client switch --env mainnet|testnet"
    exit 1
fi
...
(cd "$PKG_DIR" && sui client publish --gas-budget 350000000 --skip-dependency-verification --json) | tee "$OUTPUT_FILE"
...
for change in data.get("objectChanges", []):
    ...
    if ctype == "published":
        ...
        if obj_type.endswith("::D::Registry"):
            registry_id = obj_id
        elif obj_type.endswith("::D::OriginCap"):
            origin_cap_id = obj_id
        elif obj_type == "0x2::package::UpgradeCap":
            upgrade_cap_id = obj_id
        elif "Currency<" in obj_type and "::D::D>" in obj_type:
            currency_id = obj_id

# D v0.2.0 — External Audit R1 Submission Bundle

**Date:** 2026-04-28
**Submitter:** Rera (rera@darbitex)
**Audit round:** R1 (first external pass)
**Bundle size:** 866 LOC source + 436 LOC tests + 299 LOC self-audit

## Cover letter

D v0.2.0 is a **focused refactor + rebrand** redeploy of ONE Sui v0.1.0 (mainnet `0x9f39a102363cec6218392c2e22208b3e05972ecc87af5daa62bac7015bf3b8dc`, sealed via `package::make_immutable`).

**ONE v0.1.0 audit history:** 6 external auditor passes across 4 R2 rounds:
- Gemini 3 Pro × 2 — GREEN
- Grok × 2 — GREEN
- Claude × 1 — GREEN
- Qwen × 1 — GREEN
- DeepSeek × 1 — GREEN
- Kimi × 1 — GREEN (1 INFO Kimi-I1 about u64 add overflow on flash-repay; resolved as accepted edge case)

**0 HIGH / 0 MEDIUM / 0 LOW** across all rounds.

**Your task in this audit:** Review ONLY the V2 diff vs V1 (additive + modified surface). V1 invariants and unchanged code paths inherit prior audit conclusions. **Do not re-audit V1 logic.**
...
| Symbol | Change |
|---|---|
| `route_fee` | Split changed 25/75 → **10/90**. 10% portion now redirected to `sp_pool` as agnostic donation (joins balance, **does NOT increment `total_sp`**). 90% goes to `fee_pool` reward accumulator when keyed positions exist; cliff branch (no keyed positions) also redirects 90% to `sp_pool` agnostically. Replaces V1's instant burn with gradual burn via SP absorbs. |
| `liquidate` | Denominator for `product_factor` adjustment changed from `total_sp` to `balance::value(&sp_pool)`. New `total_sp` formula: `total_sp × (pool_before - debt) / pool_before`. `reward_index_coll` increment guarded for `total_sp == 0`. Cliff `sp_coll` redirected to `reserve_coll` instead of `sp_coll_pool`. |
| `WARNING` paragraph (4) | Rewritten for V2 mechanics (10/90 split + agnostic donation + no-dilution clarification). |

### Added (strictly additive)

| Symbol | Description |
|---|---|
| `donate_to_sp(reg, d_in, ctx)` | Permissionless D donation to SP. `balance::join(&mut sp_pool, …)`. Does NOT increment `total_sp`. Emits `SPDonated`. |
| `donate_to_reserve(reg, sui_in, ctx)` | Permissionless SUI donation to `reserve_coll`. Fortifies `redeem_from_reserve` capacity. Emits `ReserveDonated`. |
...
| Symbol | Reason |
|---|---|
| `FeeBurned` event struct | V1's 25% burn replaced by 10% donation in V2. Event no longer emitted (dead code). |
...
```
$ sui move build
[ ... only intentional W99001 lint on entry wrappers ... ]
...
Test coverage:
- 19 V1 inheritance tests still passing
- 7 new V2 tests covering donate_to_sp, donate_to_reserve, agnostic refactor, route_fee redirect, liquidation pool_before math

## Audit goals

Please verify:

1. **Math invariants preserved across V2 changes.** Critical: liquidation new `total_sp` formula maintains `total_sp = Σ(initial × current_p / snap_p)` invariant. Self-audit includes proof; please verify or refute.

2. **Agnostic donation semantics correct.** Donations bypass `total_sp` denominator → real depositors get full 90% yield (no dilution). Donations participate in liquidation absorb pro-rata via `pool_before` math. Please verify no leak/orphan pathways.

3. **Cliff path handling complete.** When `total_sp == 0` (no keyed positions): (a) route_fee 90% redirects to sp_pool, (b) liquidate sp_coll redirects to reserve_coll. Please verify no orphan accumulation.

4. **No regression in V1 invariants** at the points V2 touches them. Especially: cliff product_factor freeze (MIN_P_THRESHOLD), u64 saturation in sp_settle, sender-keyed sp_withdraw safety.

5. **Attack surface delta acceptable.** New `donate_to_sp` and `donate_to_reserve` are permissionless. Self-audit enumerates considered attack vectors A-D. Please verify or add to the list.
...
### HIGH
- [HIGH-1: brief title] — description, location, impact, recommendation
...
- [ ] reward_index_one denominator strictly keyed
...
# Self-Audit R1 (inline)

(See `audit/SELF_AUDIT_R1.md` for canonical version.)

## Executive summary

D v0.2.0 is a refactored + rebranded redeploy of ONE Sui v0.1.0. The diff is focused — V1 already passed 6 external audit rounds with 0 HIGH/MEDIUM/LOW findings. This self-audit reviews ONLY V2 changes.
...
**Verdict: GREEN.** Ready for external audit.

## Math invariant register

| Invariant | V1 | V2 |
|---|---|---|
| `sp_pool_balance ≥ Σ(position.initial × current_p / snap_p)` | ✓ | ✓ — V2 difference: donation residue counted in sp_pool but not total_sp |
| `total_sp` consistency | ✓ | ✓ — proven below |
| `total_debt = Σ(trove.debt)` | ✓ | ✓ — unchanged |
| Reward distribution proportional to keyed shares | ✓ | ✓ — denominator strictly keyed |
| Cliff guard MIN_P_THRESHOLD | ✓ | ✓ — unchanged |
| u64 saturation cap | ✓ | ✓ — unchanged |
| sender-keyed sp_withdraw blocks gift-attack escape | ✓ | ✓ — unchanged |
| sp_coll_pool ≤ Σ pending_coll claims | n/a | ✓ — fixed cliff orphan via reserve_coll redirect |

## Math correctness proof for V2 liquidate
...
This matches V2's new `total_sp` formula. ✓ Invariant preserved.

**Donation residue:**
- Pre: `sp_pool = total_sp_old + donation_residue`
- Post-burn: `sp_pool_new = pool_before - debt`
- `donation_residue_post = sp_pool_new - total_sp_new = (pool_before - debt) × donation_residue_old / pool_before`
...
### LOW (resolved before bundle)

**LOW-1: cliff liquidation orphan in sp_coll_pool** — RESOLVED

During cliff scenarios (donations in sp_pool but no keyed positions), liquidation could distribute 50% bonus SUI to `sp_coll_pool`. With `total_sp == 0`, `reward_index_coll` increment was guarded, but the SUI was still added to the pool via `balance::join`. Future depositors snap_index_coll = current_index_coll (which didn't move during cliff), so cliff-era SUI never distributed via reward index — orphan locked.

**Fix applied (line 543-549):** When `total_before == 0` at liquidation, route `sp_coll` to `reserve_coll` instead of `sp_coll_pool`. Consistent with V2 "no keyed depositors → reroute" pattern. SUI fortifies redemption reserve; no orphan.

### INFO (non-blocking)

**INFO-1: Indexer migration** — V2's `route_fee` no longer emits `FeeBurned`. Off-chain indexers built for V1 ONE need to migrate event listener to `SPDonated`. Mitigation: post-deploy migration guide.

**INFO-2: Donation u64 ceiling** — Donations grow sp_pool unboundedly within u64. At 1e8 raw decimals, max sp_pool ≈ 180 trillion D, practically unreachable. Existing u64 saturation handling on rewards already documented in WARNING (clause 2).

## Issues considered & rejected

**A.** Could donation attacker drain reserve via repeated donate-then-redeem cycles? **No** — donor net: -SUI -D -1%×D fee. Pure gift with leak.

**B.** Could agnostic donation manipulate liquidation to harm keyed depositors? **No** — donations growing sp_pool reduce per-position absorb ratio. Strict positive externality.

**C.** Could attacker frontrun open_trove with donate_to_sp to manipulate ratio? **No** — open_trove uses oracle price + collateral, not sp_pool ratio.

**D.** Could attacker frontrun donate_to_reserve to manipulate redeem_from_reserve price? **No** — redeem_from_reserve uses oracle price; donation only affects `balance::value(reserve_coll) >= coll_out` check.

---

# Source: `sources/D.move` (866 lines)
...
# Tests: `tests/D_tests.move` (436 lines)
...
1. Send this entire bundle to each auditor.
2. Each auditor reviews focused-diff, returns response in standard format above.
3. If all R1 auditors return GREEN with no new findings ≥ MEDIUM: skip R2.
4. If R1 surfaces findings: apply fixes, re-bundle as R2, re-submit.
5. Track responses in `audit/AUDIT_TRACKING.md`.

## Reference auditor history (V1 ONE)

These auditors returned GREEN on V1 — same auditors recommended for V2 continuity:

- **Gemini 3 Pro** (Google) — primary, R1 + R2 GREEN on V1
- **Grok 4** (xAI) — primary, R1 + R2 GREEN on V1
- **Claude Sonnet 4.6 / Opus 4.7** (Anthropic) — fresh-session, R2 GREEN on V1
- **Qwen3 235B Instruct** (Alibaba via Cerebras) — R2 GREEN on V1
- **DeepSeek V3** — R2 GREEN on V1
- **Kimi K2.6** (Moonshot) — R2 GREEN on V1 + 1 INFO

Recommended R1 panel: Gemini + Grok + Claude (fresh session). Optional 4th: Qwen.
